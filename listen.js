...
app.put('/new-bid/:driverid', (req, res) => {
  ...
  newBid = {ID, Status, Bidamount}
  
  query = { _id: new ObjectId(req.body.shipmentid) };
  newvalues = { $push: { bids: newBid } };
  const db = client.db('project_holder');
  console.log('pressed')
...
app.get('/bids/:shipmentid', async (req,res)=> {
  try {
    const ID = req.params.shipmentid
    const db = client.db('project_holder');
    const bids = await db.collection('shipments').findOne({ _id : new ObjectId(ID) });
    res.json(bids);
  }
  catch (error) {
